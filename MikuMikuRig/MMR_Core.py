...
def RIG(context):
    ...
    add_constraint("ORG-Arm_L","Arm_L",True)
    add_constraint("ORG-Arm_R","Arm_R",True)
    add_constraint("ORG-Elbow_L","Elbow_L",True)
    add_constraint("ORG-Elbow_R","Elbow_R",True)
    add_constraint("ORG-Shoulder_L","Shoulder_L",True)
    add_constraint("ORG-Shoulder_R","Shoulder_R",True)
    add_constraint("ORG-Wrist_L","Wrist_L",True)
    add_constraint("ORG-Wrist_R","Wrist_R",True)

    add_constraint("ORG-Leg_L","Leg_L",True)
    add_constraint("ORG-Leg_R","Leg_R",True)
    add_constraint("ORG-Knee_L","Knee_L",True)
    add_constraint("ORG-Knee_R","Knee_R",True)

    add_constraint("DEF-Ankle_L","Ankle_L",True)
    add_constraint("DEF-Ankle_R","Ankle_R",True)
    add_constraint("DEF-Ankle_L","LegIK_L",False)
    add_constraint("DEF-Ankle_R","LegIK_R",False)
    add_constraint("DEF-ToeTipIK_L","ToeTipIK_L",False)
    add_constraint("DEF-ToeTipIK_R","ToeTipIK_R",False)

    #修正缺少UpperBody2的骨骼
    #fix the armature that lack upperbody2
    if no_UpperBody2:
        add_constraint("UpperBody2_fk","UpperBody",True)
    else:
        add_constraint("UpperBody2_fk","UpperBody2",True)
        add_constraint("UpperBody_fk","UpperBody",True)

    add_constraint("LowerBody_fk","LowerBody",True)
    add_constraint("torso","Center",True)
    add_constraint("DEF-Neck","Neck",True)
    add_constraint("DEF-Head","Head",True)
    add_constraint("root","ParentNode",True)

    #add_constraint("ORG-eye.L","Eye_L",True)
    #add_constraint("ORG-eye.R","Eye_R",True)


    add_constraint("ORG-Thumb0_L","Thumb0_L",True)
    add_constraint("ORG-Thumb0_R","Thumb0_R",True)
    add_constraint("ORG-Thumb1_L","Thumb1_L",True)
    add_constraint("ORG-Thumb1_R","Thumb1_R",True)
    add_constraint("ORG-Thumb2_L","Thumb2_L",True)
    add_constraint("ORG-Thumb2_R","Thumb2_R",True)

    add_constraint("ORG-IndexFinger1_L","IndexFinger1_L",True)
    add_constraint("ORG-IndexFinger1_R","IndexFinger1_R",True)
    add_constraint("ORG-IndexFinger2_L","IndexFinger2_L",True)
    add_constraint("ORG-IndexFinger2_R","IndexFinger2_R",True)
    add_constraint("ORG-IndexFinger3_L","IndexFinger3_L",True)
    add_constraint("ORG-IndexFinger3_R","IndexFinger3_R",True)
    add_constraint("ORG-MiddleFinger1_L","MiddleFinger1_L",True)
    add_constraint("ORG-MiddleFinger1_R","MiddleFinger1_R",True)
    add_constraint("ORG-MiddleFinger2_L","MiddleFinger2_L",True)
    add_constraint("ORG-MiddleFinger2_R","MiddleFinger2_R",True)
    add_constraint("ORG-MiddleFinger3_L","MiddleFinger3_L",True)
    add_constraint("ORG-MiddleFinger3_R","MiddleFinger3_R",True)
    add_constraint("ORG-RingFinger1_L","RingFinger1_L",True)
    add_constraint("ORG-RingFinger1_R","RingFinger1_R",True)
    add_constraint("ORG-RingFinger2_L","RingFinger2_L",True)
    add_constraint("ORG-RingFinger2_R","RingFinger2_R",True)
    add_constraint("ORG-RingFinger3_L","RingFinger3_L",True)
    add_constraint("ORG-RingFinger3_R","RingFinger3_R",True)
    add_constraint("ORG-LittleFinger1_L","LittleFinger1_L",True)
    add_constraint("ORG-LittleFinger1_R","LittleFinger1_R",True)
    add_constraint("ORG-LittleFinger2_L","LittleFinger2_L",True)
    add_constraint("ORG-LittleFinger2_R","LittleFinger2_R",True)
    add_constraint("ORG-LittleFinger3_L","LittleFinger3_L",True)
    add_constraint("ORG-LittleFinger3_R","LittleFinger3_R",True)

    add_constraint_execute()
...
#设置最小ik迭代
def set_min_ik_loop(min_ik_loop=10):
    if check_arm()==False:
        return
    for bone in mmd_arm.pose.bones:
...
#重定向mixamo动画
def retarget_mixmao(context,mixamo_path):
    ...
    ik_fk_hand=mmr_property.ik_fk_hand
    ik_fk_leg=mmr_property.ik_fk_leg
...
def load_vmd(context,vmd_path,rigify_arm,fade_in_out,action_scale=1,debug=False):
    if rigify_arm.type!='ARMATURE':
        return(False)
    if vmd_path==None:
        return(False)
...
def convert_rigid_body_to_cloth(mmr_property):
